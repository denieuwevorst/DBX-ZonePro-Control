(() => {
  ...
  function connectWs() {
    ...
    ws.addEventListener('message', (event) => {
      ...
      if (msg.type === 'state') {
        ...
        applyState(msg.state, currentConfig);
      } else if (msg.type === 'status') {
        setLink(!!msg.connected);
      } else if (msg.type === 'config') {
        // Settings were changed and saved from /config -- rebuild the
        // strips in place so every open tab reflects the new zones/inputs
        // without needing a manual refresh.
        currentConfig = msg.config;
        buildStrips(currentConfig);
      }
    });
  }
  ...
  async function init() {
    currentConfig = await fetch('/api/config').then((r) => r.json());
    buildStrips(currentConfig);

    if (strips.size > 0) {
      const initial = await fetch('/api/state').then((r) => r.json());
      applyState(initial.state, currentConfig);
